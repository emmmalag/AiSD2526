...
Node* buildTernaryHuffman(unordered_map<char, int>& freq) {
...
void getTernaryCodes(Node* root, string code, unordered_map<char, string>& codes) {
    if (!root) return;
    ...
    for (size_t i = 0; i < root->children.size(); i++) {
        getTernaryCodes(root->children[i], code + to_string(i), codes);
    }
}
...
int main() {
    ...
    for (int k = 0; k < 5; k++) {
        ...
        for (int n : {500000, 1000000, 1500000, 2000000, 2500000, 3000000, 3500000, 4000000, 4500000, 5000000, 5500000, 6000000, 6500000, 7000000, 7500000, 8000000, 8500000, 9000000, 9500000, 10000000}) {
            ...
            Node* root = buildTernaryHuffman(freq);
